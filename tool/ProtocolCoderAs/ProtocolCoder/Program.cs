...
;
        ...
        static bool AnalyzeInputDocument(string filename)
        {
            try
            {
                ...
                Variables["FactoryName"] = root.Attributes["name"].Value;
                ...
                if (IncFiles.Length > 0)
                    foreach (string str in IncFiles)
                        Variables["Include"] += string.Format("import {0};\n", str);
                else
                    Variables["Include"] = "";
        ...
        static void GenerateProtocolBuffersFiles()
        {
            ...
            {
                sw.Write("package protobuf;\n");
                sw.Write(szProtocolBufferContent);
            }
            string arguments = String.Format("--plugin=protoc-gen-as3=\"as_plugin\\protoc-gen-as3.bat\" --as3_out=. {0}", 
            ...
            //System.IO.File.Move(System.IO.Path.GetFileNameWithoutExtension(szOutputProtoFile) + ".as",
            //    szOutputDirectory + System.IO.Path.GetFileNameWithoutExtension(szOutputProtoFile) + ".as");
        }
