...
;
        ...
        static bool AnalyzeInputDocument(string filename)
        {
            try
            {
                ...
                string strProcessorClass = typenode.Attributes["processor"].Value;
                Variables["ProcessorClass"] = strProcessorClass;
                if (strProcessorClass.Contains("::"))
                    IsRawPoint = false;
                ...
                if (IncFiles.Length > 0)
                    foreach (string str in IncFiles)
                        Variables["Include"] += string.Format("#include \"{0}\"\n", str);
                else
                    Variables["Include"] = "";
        ...
        static void GenerateProtocolBuffersFiles()
        {
            ...
            string arguments = String.Format("--cpp_out=. {0}", 
                                                System.IO.Path.GetFileName(szOutputProtoFile));
            ProcessStartInfo startInfo = new ProcessStartInfo("protoc.exe", arguments);
            ...
            System.IO.File.Move(System.IO.Path.GetFileNameWithoutExtension(szOutputProtoFile)+".pb.cc",
                szOutputDirectory + System.IO.Path.GetFileNameWithoutExtension(szOutputProtoFile) + ".pb.cc");
            System.IO.File.Move(System.IO.Path.GetFileNameWithoutExtension(szOutputProtoFile) + ".pb.h",
                szOutputDirectory + System.IO.Path.GetFileNameWithoutExtension(szOutputProtoFile) + ".pb.h");

            string pbccFileName = szOutputDirectory + System.IO.Path.GetFileNameWithoutExtension(szOutputProtoFile) + ".pb.cc";
            string str = "";
            using (StreamReader sw = new StreamReader(pbccFileName))
            {
                str = sw.ReadLine();
                str += "\n#include \"stdafx.h\"";
                str += sw.ReadToEnd();
            }
            using (StreamWriter sw = new StreamWriter(pbccFileName, false))
            {
                sw.Write(str);
            }
            //using (FileStream fs = File.Open(pbccFileName, FileMode.Open, FileAccess.ReadWrite))
            //{
            //    fs.Read()
            //    if (fs.CanSeek)
            //    {
            ...
            //        string str = "include \"stdafx.h\"\r\n";
            //        byte [] info = Encoding.GetEncoding("gb2312").GetBytes(str);
        ...
        static void Main(string[] args)
        {
            try
            {
                ...
                Generator generator = new Generator(szOutputDirectory, Variables, MessageInfosFromName, IsServerProto, IsRawPoint);
                generator.Run();
            }
            catch (System.Exception e)
            {
